...
class Face_Register:
    ...
    def check_existing_faces_cnt(self):
        # Start from person_1 / Start from person_x+1
        if os.listdir("data/data_faces_from_camera/"):
            ...
            for person in person_list:
                ...
                # It checks if the folder/file name starts with "person_"
                # before trying to process it. This ignores files like '.DS_Store'.
                if person.startswith("person_"):
                    try:
                        person_order = person.split('_')[1]
                        person_num_list.append(int(person_order))
                    except (IndexError, ValueError):
                        # Handles malformed folder names gracefully
                        print(f"Skipping malformed folder name: {person}")
                        continue
    ...
    def save_current_face(self):
        if self.face_folder_created_flag:
            ...
            self.log_all["text"] = "Please run step 2 to create a folder first!"
